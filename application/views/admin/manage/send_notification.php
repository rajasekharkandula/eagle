...
<body>
				...
				<div class="col-md-9">
					<div class="box">
						<form id="category_form">
							<div class="row">
								<label class="col-md-3 text-right">Subject <span>*</span></label>
								<div class="col-md-6">
									<input type="text" name="subject" id="subject"> 
								</div>
							</div>
							<div class="row mt-10">
								<label class="col-md-3 text-right">Content <span>*</span></label>
								<div class="col-md-6">
									<textarea name="content" id="content"></textarea> 
								</div>
							</div>	
							<div class="row assign-lbl assign_type">
								<div class="col-md-3 text-right">Send to </div>
								<div class="col-md-2">
							...
							<div class="text-center mb-20">
								<button class="btn" type="button" id="submit_btn">Send</button>
	...
	$('#submit_btn').on('click',function(){
		...
		if($("#subject").val().trim() == ""){
			error++;
			$("#subject").parent().append('<div class="text-danger">This field is required</div>');
		}
		if($("#content").val().trim() == ""){
			error++;
			$("#content").parent().append('<div class="text-danger">This field is required</div>');
		}
		if(error ==0){
			$('#submit_btn').attr("disabled",true);
			$('#submit_btn').html('<i class="fa fa-refresh spin"></i> Please wait...');
			$.ajax({
				url:'<?php echo base_url('admin/ins_upd_notification'); ?>',
				type:'POST',
				data: {'type':'HR_SEND','assign_type':assign_type,'subject':$("#subject").val(),'content':$("#content").val(),'group_id':$("#groups").val(),'designation_id':$("#designations").val(),'users':users},
				dataType:'JSON'
			}).done(function(data){
				if(data.status == 1){
